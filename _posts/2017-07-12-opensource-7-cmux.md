...
### 검색 / 필터 단계
CMUX의 꽃에 해당되는 부분입니다. 빔(Vim)신이라고 불리는 카카오의 [최준건](https://github.com/junegunn)이 만드신 [FZF](https://github.com/junegunn/fzf)라는 커맨드라인 검색 도구를 사용하여, 원하는 조건으로 입력을 검색하고 결과를 걸러냅니다.

### 명령어 생성 단계
걸러진 결과를 바탕으로 명령어 집합을 생성합니다. CMUX는 [Ruby](https://www.ruby-lang.org)로 작성되어 있기 때문에 당연히 Ruby 코드로 생성하겠죠.

### 명령어 실행 단계
생성한 명령어 집합을 실행합니다. 이 때 명령어 집합을 병렬로 실행해야 할 수도 있는데, 이를 위해 [TMUX](https://github.com/tmux/tmux/wiki)를 사용합니다.
...
CMUX 코드는 Ruby로 작성되어 있지만, 이 예제에서는 쉘 스크립트를 사용하겠습니다. 또한 JSON 포맷으로 제공되는 [Cloudera Manager REST API](http://cloudera.github.io/cm_api/apidocs/v16/)를 활용하기 편해지도록 [jq](https://stedolan.github.io/jq/)를 설치하여 사용하겠습니다.

```sh
brew install jq
```

자 이제 준비가 되었으니 먼저 데이터 입력 부분을 구현해 볼까요?
...
> 참고
> 
> CMUX에서는 결과물을 잘 정렬된 테이블 형태로 출력할 수 있도록, 이 부분이 [별도의 모듈](https://github.com/kakao/cmux/blob/master/lib/cmux/utils/formatter.rb#L7-L31)로 구현하였습니다.
...
### 명령어 생성
위 결과의 마지막 열인 "Custer URL"을 기본 브라우저로 열 수 있도록 명령어를 만들겠습니다. OSX 환경이라는 가정하에 `open` 명령어를 사용합니다.
...
## 명령어 실행
마지막으로 위에서 생성한 명령어를 실행할 수 있도록 스크립트를 변경하겠습니다.
...
> 참고
> 
> CMUX에서는 [tws](https://github.com/kakao/cmux/blob/master/lib/cmux/commands/tmux_window_splitter.rb) 라는 별도의 명령어로 보다 정밀한 화면 제어가 가능하도록 구현되어 있습니다.
...
마지막으로 CMUX를 위해 [FZF](https://github.com/junegunn/fzf)에 여러가지 기능을 탑재해 주신 [최준건](https://github.com/junegunn)님께 감사의 말씀을 드립니다.
